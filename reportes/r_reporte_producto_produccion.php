...
class PDF extends tFPDF{
        ...
        function Header(){
           ...
           // Salto de línea
            ...
            $this->Cell(80);
            // Título
            $this->Cell(30,10,'REPORTE DE PRODUCCION POR PRODUCTO',0,0,'C');
            // Salto de línea
            $this->Ln(10);
            ...
            if($result[0]=='1'){
                ...
                foreach ($result[2]  as $row) {
                   ...
                   // $tipo = ($row['tipo_persona']==2) ? "Empleado": "Administrador"; 
                    $GLOBALS['total_pro'] = $row['int_cantidad'] * $row['dou_costo_botella'];
                    $this->Cell($w[0],6,dateformateado($row['dat_fecha_vencimiento_botella']),'LR',0,'C',$fill);
                    $this->Cell($w[1],6,$row['int_cantidad'],'LR',0,'C',$fill);
                    $this->Cell($w[2],6,"$".number_format($row['dou_costo_botella'],2),'LR',0,'C',$fill);
                    $this->Cell($w[3],6,"$".number_format($GLOBALS['total_pro'],2),'LR',0,'C',$fill);
                     $this->Ln();
                    $fill = !$fill;
    ...
    $header = array('Fecha Vencimiento', 'Cantidad', 'Costo', 'Total');
    ...
    $pdf->AliasNbPages();
    $pdf->AddPage();
    $pdf->Cell(70,10,"Producto: ".$result[2][0]['nva_nom_producto'],0,0,'C');
    $pdf->Ln(10);
